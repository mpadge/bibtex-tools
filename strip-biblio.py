...
def dump_cites (ref_list):
    """
    Dump a list of the bibtex cite keys to the file "refs.txt", which can then
    be used in "make_lit_review" to dump the formatted reviews of each cited
    paper.
    """
...
def get_citations (ref_list):
    """
    Find citation keys in .bib file and copy citations. This presumes citations
    are indeed in the .bib file, and will crash if not. Not having them there
    would nevertheless be rather strange.

    The routine may produce unpredictable results if non-encoding characters are
    encountered, such as sometimes happens with copy-pasted entries. The output
    thus must be examined to identify such, and the corresponding entries in the
    original .bib file modified. It's much easier that way than modifying this
    actual program to potentially flag all such cases (there should generally be
    very few).

    A further issue is single quotations marks (') in titles and author names,
    which are also not yet handled correctly.

    The extraction of fields is done with the new (Oct 2013) regex search, which
    allows recursive searching for nested expressions. Bibtex files often have
    nested {}, which are parsed with the regex.search expression below. The
    commands work thus:
    ...
    """
    There is no matching backwards with regex, so positions of all references
    are first put in an index. The find in this case is "@" followed by anything
    ("." for anything, and "*" for unspecified number of repeats) and finishing
    in "{". This is because the "@" symbol also appears in email addresses, but
    all of these cases will then first end with "}". Actually, i'm note sure
    exactly why this works at all ...
    """
...
ref_list = extract_cites ("text.tex")
dump_cites (ref_list)
get_citations (ref_list)
